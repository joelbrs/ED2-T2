...
public class RBTree<T extends Comparable<T>> { //Fonte: https://github.com/iamareebjamal/RedBlackTree-GUI/blob/master/src/areeb/trees/rbt/RedBlackTree.java
    ...
    public void insert(T item) {
        ...
        this.root.setColor(RBColorEnum.BLACK);
        ++this.size;
    }
    ...
    private void balanceAfterInsert(RBNode<T> node) {
        ...
        if (RBNode.isRed(node.getUncle())) {
            ...
            RBNode.setColor(node.getParent(), RBColorEnum.BLACK);
            RBNode.setColor(node.getGrandparent(), RBColorEnum.RED);
    ...
    private void balanceAfterDelete(RBNode<T> node) {
        while (node != this.root && node.isBlack()) {
            ...
            if (node == RBNode.getLeft(node.getParent())) {
                if (RBNode.isRed(sibling)) {
                    RBNode.setColor(sibling, RBColorEnum.BLACK);
                    RBNode.setColor(node.getParent(), RBColorEnum.RED);
                ...
                if (RBNode.isBlack(RBNode.getLeft(sibling)) && RBNode.isBlack(RBNode.getRight(sibling))) {
                    RBNode.setColor(sibling, RBColorEnum.RED);
                    node = node.getParent();
                    continue;
                }
                if (RBNode.isBlack(RBNode.getRight(sibling))) {
                    RBNode.setColor(RBNode.getLeft(sibling), RBColorEnum.BLACK);
                    RBNode.setColor(sibling, RBColorEnum.RED);
                ...
                RBNode.setColor(node.getParent(), RBColorEnum.BLACK);
                RBNode.setColor(RBNode.getRight(sibling), RBColorEnum.BLACK);
                this.rotateLeft(node.getParent());
                node = this.root;
                continue;
            }
        ...
        RBNode.setColor(node, RBColorEnum.BLACK);
    }
    ...
    private boolean hasRightParent(RBNode<T> node) {
        return RBNode.getRight(node.getGrandparent()) == node.getParent();
    }

    private boolean hasLeftParent(RBNode<T> node) {
        return RBNode.getLeft(node.getGrandparent()) == node.getParent();
    }

    private boolean isRightChild(RBNode<T> node) {
        return RBNode.getRight(node.getParent()) == node;
    }

    private boolean isLeftChild(RBNode<T> node) {
        return RBNode.getLeft(node.getParent()) == node;
    }
}
